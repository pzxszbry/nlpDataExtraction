...
class Solution:
    ...
    def main(self, input_path, output_path):
        # self.file = open("output/coref_resolved/AppleInc_coref.txt")
        ...
        files= os.listdir(os.path.join(input_path, 'coref_resolved'))
        for file in files:
            file_name, file_ext = os.path.splitext(file)
            result_root_path = os.path.join(output_path, file_name.replace('no_coref', 'result'))
            self.file = open(os.path.join(input_path, 'coref_resolved', file), 'r', encoding='utf-8-sig')
            fl = self.file.read()
            ...
            # print(sentences[0])
            self.sentenceFile = open(os.path.join(result_root_path, "sentenceFile.txt"),"w", encoding='utf-8-sig')
            ...
            self.tokenFile = open(os.path.join(result_root_path, "tokenFile.txt"),"w", encoding='utf-8-sig')
            ...
            self.lemmaFile = open(os.path.join(result_root_path, "lemmaFile.txt"),"w", encoding='utf-8-sig')
            ...
            self.POSFile = open(os.path.join(result_root_path, "POSFile.txt"),"w", encoding='utf-8-sig')
            self.POSFile.write(str(pos_tag))
            self.POSFile.close()

            self.parseFile = open(os.path.join(result_root_path, "parseFile.txt"),"w", encoding='utf-8-sig')
            parseTree = self.getParseTree(sentences)
            self.parseFile.write(str(parseTree))
            self.parseFile.close()

            self.wordNetFile = open(os.path.join(result_root_path, "wordNet.json"),"w", encoding='utf-8-sig')
            ...
            outputfile = open(os.path.join(result_root_path, 'dep.html'),"w", encoding='utf-8-sig')
            outputfile.write(html)
            outputfile.close()

            html2 = displacy.render(doc,style='ent',page=True)
            outputfile2 = open(os.path.join(result_root_path, 'ent.html'),"w", encoding='utf-8-sig')
            outputfile2.write(html2)
            outputfile2.close()

            self.outputjson = open(os.path.join(result_root_path, "outputjson.json"),"w", encoding='utf-8-sig')
